...
const MessageContainer = () => {
  ...
  <div className={`${message.sender===selectedChatData._id?"text-left":"text-right"}`}>
    {
      message.messageType === "text" && (
        <div className={`${message.sender !== selectedChatData._id? "bg-[#1c1917]/5 text-[#dc2626]/90 border-[#dc2626]/50":"bg-[#1c1917]/5 text-white border-[#ffffff]/20"}
  border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>{message.content}</div>
      )
    }
    {
      message.messageType==="file" &&  <div className={`${message.sender !== selectedChatData._id? "bg-[#1c1917]/5 text-[#dc2626]/90 border-[#dc2626]/50":"bg-[]/5#1c1917 text-white border-[#ffffff]/20"}
  ...
  const renderChannelMessages = (message) => {
    return (
      <div className={`mt-5 ${message.sender._id !== userInfo.id ? "text-left" : "text-right"}`}>
        {
      message.messageType === "text" && (
        <div className={`${message.sender._id === userInfo.id? "bg-[#1c1917]/5 text-[#dc2626]/90 border-[#dc2626]/50":"bg-[#1c1917]/5 text-white border-[#ffffff]/20"}
  border inline-block p-4 rounded my-1 max-w-[50%] break-words ml-10`}>{message.content}</div>
      )
    }
     {
      message.messageType==="file" &&  <div className={`${message.sender._id === userInfo.id? "bg-[]/5 text-[#dc2626]/90 border-[#dc2626]/50":"bg-[#1c1917]/5 text-white border-[#ffffff]/20"}
      border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>
        {checkIfImage(message.fileUrl)? ( 
          <div className="cursor-pointer"
          onClick={() => {
    ...
    }
    {
      message.sender._id !== userInfo.id ? <div className="flex items-center justify-start gap-3">
        <Avatar className="h-8 w-8 rounded-full overflow-hidden">
              ...
              <span className="text-xs text-white/50">{moment (message.timestamp).format("LT")}</span>
      </div>: <div className="text-xs text-white/50 mt-1">{moment (message.timestamp).format("LT")}</div>
    }
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-hidden p-4 px-8 md:w-[65vw] lg:w-[70vw] xl:w-[80vw] w-full bg-slate-950 ">
        {renderMessages()}
        <div ref={scrollRef} />
        {
